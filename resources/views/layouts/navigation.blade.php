    ...
    <div class="flex h-full flex-col">
        ...
        <div class="flex-1 space-y-1 overflow-y-visible px-3 py-4">
            ...
            @if(Auth::check() && in_array($role, ['admin', 'pimpinan']))
                ...
                <a href="{{ route('dokumen.pimpinan') }}" class="{{ request()->routeIs('dokumen.pimpinan') ? 'bg-red-50 text-red-600 font-semibold' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900' }} flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm transition">
                    <svg class="h-4 w-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3"/></svg>
                    <span>Dokumen</span>
                </a>
            @endif
            ...
            @if(Auth::check() && in_array($role, ['admin', 'pimpinan']))
                ...
                <a href="{{ route('format.index') }}" class="{{ request()->routeIs('format.*') ? 'bg-red-50 text-red-600 font-semibold' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900' }} flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm transition">
                    <svg class="h-4 w-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 3v5h5"/></svg>
